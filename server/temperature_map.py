...
def avgTempMatrix(vert_res, horiz_res, rect, month, category):
    ...
    qset = Station.objects.raw_query(
        {'coordinates': {'$within': {'$box': rect}}})

    lnggrid, latgrid = np.mgrid[minlng:maxlng:horiz_res*5,
                                minlat:maxlat:vert_res*5]
    ...
    while True:
        try:
            ...
            response_str = urllib.urlopen(
                "http://ned.usgs.gov/epqs/pqs.php?" + qs).read()
...
def elevationMatrix(res, rect):
    ...
    for i in range(nt):
        ...
        lnggrid, latgrid = np.mgrid[minlng + i*dlng/nt: minlng + (i+1)*dlng/nt:res/nt,
                                    minlat:maxlat:res]
        threads[i] = Thread(target=doQueries, args=(
            lnggrid, latgrid, pointsList, valuesList, i))
        threads[i].start()
...
for month, cat in itertools.product(months, c):
    gc.collect()
    interpolated_elev, interpolated_temp = avgTempMatrix(
        vert_res, horiz_res, rect, month, cat)
    gc.collect()
    result = modeledTemperature(
        interpolated_elev, interpolated_temp, actual_elev)
    with open('maps/' + str(month) + '_' + str(cat) + '.png', 'w+') as fd:
        plt.imsave(fd, result, format='png')
